...
object OverpassApiClient {
    ...
    // ── Simple in-memory cache: avoid re-fetching the same city twice ─────────
    // Key = query string (lowercased), Value = list of restaurants
    ...
    suspend fun getRestaurants(searchQuery: String): List<Restaurant> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                // ── Upgrade 1: Return cached result if available ──────────────
                cache[cacheKey]?.let {
                    Log.d(TAG, "Cache hit for '$query' (${it.size} items)")
                ...
                val firstAttempt = geocode(query)
                if (firstAttempt != null) {
                    lat = firstAttempt.first; lon = firstAttempt.second
                    resolvedName = firstAttempt.third
                } else {
                    val fallback = query
                        .replace(Regex("(?i)restaurants?|food|near|in|around"), "")
                        .trim()
                    val secondAttempt = geocode(fallback.ifBlank { query })
                        ?: return@withContext emptyList<Restaurant>().also {
                            Log.e(TAG, "Nominatim: no result for '$query'")
                        }
                    lat = secondAttempt.first; lon = secondAttempt.second
                    resolvedName = secondAttempt.third
                }
                ...
                val postBytes = "data=${URLEncoder.encode(overpassQuery, "UTF-8")}".toByteArray()
                val conn = (URL(OVERPASS_URL).openConnection() as HttpURLConnection).apply {
                    requestMethod = "POST"; doOutput = true
                    connectTimeout = 20000; readTimeout = 50000
                    setRequestProperty("Content-Type", "application/x-www-form-urlencoded")
                    setRequestProperty("Content-Length", postBytes.size.toString())
                    setRequestProperty("User-Agent", "FoodFinderApp/2.0 (android)")
                }
                ...
                for (i in 0 until elements.length()) {
                    ...
                    // ── Upgrade 2: Parse REAL fields from Overpass tags ───────
                    val phone        = tags?.optString("contact:phone",  "").orEmpty()
                        .ifBlank { tags?.optString("phone", "").orEmpty() }
                    val website      = tags?.optString("website",        "").orEmpty()
                        .ifBlank { tags?.optString("contact:website", "").orEmpty() }
                    val openingHours = tags?.optString("opening_hours",  "").orEmpty()
                    val ratingStr    = tags?.optString("stars:quality",  "").orEmpty()
    ...
    // ── Upgrade 3: reverseGeocode — lat/lon → city name ──────────────────────
    // Called from MainActivity after GPS gives us coordinates
    ...
    // ── Nominatim forward geocoding (unchanged) ───────────────────────────────
    private fun geocode(query: String): Triple<String, String, String>? {
        return try {
            val encoded = URLEncoder.encode(query, "UTF-8")
            val url = "https://nominatim.openstreetmap.org/search" +
            ...
            Log.e(TAG, "geocode('$query') failed: ${e.message}"); null
        }
    }

    // ── Menu generator (unchanged — still a placeholder until real menu APIs) ─
    private fun generateCuisineMenu(cuisine: String, name: String): List<FoodItem> {
        val c = cuisine.lowercase(); val n = name.lowercase()
        return when {
            c.contains("pizza") || n.contains("pizza") || c.contains("italian") -> listOf(
                FoodItem("Margherita Pizza", "₹349"), FoodItem("Pepperoni Feast", "₹449"),
                FoodItem("Pasta Carbonara",  "₹329"), FoodItem("Garlic Bread",    "₹149"))
            c.contains("indian") || c.contains("biryani") || n.contains("biryani")
                    || n.contains("dhaba") || n.contains("punjabi") -> listOf(
                FoodItem("Chicken Biryani",      "₹299"), FoodItem("Paneer Butter Masala", "₹249"),
                FoodItem("Butter Naan",          "₹49"),  FoodItem("Gulab Jamun",           "₹99"))
            c.contains("burger") || n.contains("burger") || c.contains("american") -> listOf(
                FoodItem("Classic Cheeseburger", "₹199"), FoodItem("Bacon Double Burger",  "₹299"),
                FoodItem("Fries & Coke",         "₹129"), FoodItem("Onion Rings",           "₹99"))
            c.contains("chinese") || c.contains("asian") -> listOf(
                FoodItem("Kung Pao Chicken",     "₹279"), FoodItem("Vegetable Chow Mein",  "₹199"),
                FoodItem("Spring Rolls (4 pcs)", "₹149"), FoodItem("Fried Rice",           "₹179"))
            c.contains("coffee") || n.contains("cafe") || n.contains("coffee") -> listOf(
                FoodItem("Cappuccino", "₹149"), FoodItem("Cold Coffee",    "₹179"),
                FoodItem("Croissant",  "₹129"), FoodItem("Chocolate Cake", "₹199"))
            else -> listOf(
                FoodItem("Chef's Special",     "₹349"), FoodItem("Fresh Garden Salad", "₹199"),
                FoodItem("Soup of the Day",    "₹149"), FoodItem("Cold Beverage",       "₹79"))
        }
    }
}
